...
import { Button, Container, Row, Col, Accordion, Navbar, Nav, ListGroup, Form } from "react-bootstrap";
		...
		return (
			<ListGroup.Item>
				<Row>
					...
					<Col xs={2}>
						<Form.Check checked={payList[idx]} onChange={(e)=>handleCheck(e.target.checked, idx)}/>
					</Col>
				</Row>
			</ListGroup.Item>
		)
	...
	return (
		<Container className="container-home">
			...
			<Accordion>
				<Row>
					<Col xs={6}>
						<Button variant={paymentState == "myself" ? "secondary" : "outline-secondary"}
					...
					<Col xs={6}>
						<Accordion.Toggle as={Button} variant="link" eventKey="0" variant={paymentState == "other" ? "primary" : "outline-primary"}
			...
			<Navbar variant="dark" bg="secondary" fixed="bottom" style={{width: "100%"}}>
				<Nav className="justify-content-center text-center" style={{width: "100%"}}>
					<Nav.Link style={{width: "100%"}} href="#payment/final" onClick={handleNext} disabled={paymentState === "none"}>
						<h5 className="text-white" style={{margin: "unset"}}>進入結帳</h5>
					</Nav.Link>
				</Nav>
			</Navbar>
		</Container>
...
export default connect(mapStateToProps, { setPaymentList })(PaymentPersonal);
