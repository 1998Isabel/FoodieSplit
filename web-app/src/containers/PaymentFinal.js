...
function PaymentFinal(props) {
            ...
            <div key={idx}>
                <Navbar variant="light" bg="light" sticky="top" >
                    <Card.Title className="navbar-checkout-all">
                        ...
                        <div style={{ position: "absolute", right: "8%" }}>個人金額：${ele.totalPrice}</div>
                    </Card.Title>
                </Navbar>
    ...
    return (
        <div>
            ...
            >
                <h5>總共結帳金額: ${usersOrder.map(e => (e.totalPrice)).reduce(reducer, 0)}</h5>
            </div>
            <Navbar variant="dark" bg="primary" fixed="bottom" style={{width: "100%"}}>
				<Nav className="justify-content-center text-center" style={{width: "100%"}}>
					<Nav.Link style={{width: "100%"}} href="#payment/success" onClick={handleComfirm}>
						<h5 className="text-white" style={{margin: "unset"}}>LINE PAY 轉帳給店家</h5>
					</Nav.Link>
				</Nav>
			</Navbar>
        </div>
...
export default connect(mapStateToProps, null)(PaymentFinal);